...
def _get_pool() -> SimpleConnectionPool:
    ...
    if _pool is None:
        if not DATABASE_URL:
            raise RuntimeError(
                "DATABASE_URL is not set. Add a Postgres connection string as an "
                "environment variable (Neon, Supabase, or Vercel Postgres all work)."
            )
        # Small pool: serverless functions are short-lived and single-request-ish,
        # so we don't want to hold many idle connections open per instance.
        _pool = SimpleConnectionPool(1, 5, dsn=DATABASE_URL, sslmode="require")
    return _pool
...
def db_create_user(business_name, email, password):
    ...
    try:
        with get_cursor(commit=True) as cur:
            cur.execute(
                "INSERT INTO users (business_name, email, password_hash, password_salt) "
...
def db_add_customer(user_id, name, mobile):
    try:
        with get_cursor(commit=True) as cur:
            cur.execute(
                "INSERT INTO customers (user_id, name, mobile) VALUES (%s, %s, %s) RETURNING id",
                (user_id, name, mobile),
            )
            return cur.fetchone()["id"]
    except psycopg2.IntegrityError:
    ...
    with get_cursor() as cur:
        cur.execute("""
            SELECT customers.id, customers.name, customers.mobile,
                   ...
                   COALESCE(SUM(bills.paid_amount), 0) AS total_paid,
                   COALESCE(SUM(bills.balance_due), 0) AS total_due,
...
def db_delete_product(user_id, product_id):
    with get_cursor(commit=True) as cur:
        ...
        if cur.fetchone() is None:
            raise KeyError("product not found")
        cur.execute("SELECT COUNT(*) AS n FROM bill_items WHERE product_id = %s", (product_id,))
        if cur.fetchone()["n"]:
...
def db_create_bill(user_id, customer_id, items, paid_amount, payment_mode):
    with get_cursor(commit=True) as cur:
        if customer_id is not None:
            cur.execute("SELECT id FROM customers WHERE id = %s AND user_id = %s", (customer_id, user_id))
            if cur.fetchone() is None:
        ...
        if paid_amount > 0:
            cur.execute(
                "INSERT INTO payments (bill_id, amount, mode) VALUES (%s, %s, %s)",
...
def db_add_payment(user_id, bill_id, amount, mode):
    with get_cursor(commit=True) as cur:
        cur.execute("SELECT balance_due FROM bills WHERE id = %s AND user_id = %s", (bill_id, user_id))
        if cur.fetchone() is None:
...
def db_get_bills(user_id, start_date=None, end_date=None, customer_id=None):
    ...
    params = [user_id]
    if start_date:
        query += " AND bills.bill_date::date >= %s::date"
...
def db_get_home_summary(user_id):
    with get_cursor() as cur:
        cur.execute("SELECT COUNT(*) AS n FROM customers WHERE user_id = %s", (user_id,))
        total_customers = cur.fetchone()["n"]
        cur.execute("SELECT COUNT(*) AS n FROM products WHERE user_id = %s", (user_id,))
        total_products = cur.fetchone()["n"]
        cur.execute("SELECT COUNT(*) AS n FROM bills WHERE user_id = %s", (user_id,))
        total_bills = cur.fetchone()["n"]
...
def ensure_demo_account():
    ...
    if user_id is None:
        return  # created concurrently by another cold start; fine

    demo_products = [
        ("Rice 5kg",            250.0, 320.0, 40, 10),
        ("Sunflower Oil 1L",    140.0, 175.0, 25, 8),
        ("Wheat Flour 5kg",     210.0, 260.0, 4,  10),   # intentionally low stock
        ("Sugar 1kg",           42.0,  55.0,  60, 15),
        ("Tea Leaves 250g",     90.0,  120.0, 30, 10),
        ("Toor Dal 1kg",        110.0, 140.0, 2,  8),    # intentionally low stock
    ]
    product_ids = {}
    for name, cost, sell, qty, reorder in demo_products:
        product_ids[name] = db_add_product(user_id, name, cost, sell, qty, reorder)
...
def call_groq(system_prompt: str, user_prompt: str, max_tokens: int = 600) -> str:
    api_key = get_api_key()
    if not api_key:
        return ("No Groq API key configured. Set the GROQ_API_KEY environment "
...
def generate_daily_brief(user_id: int) -> str:
    """A short morning-style summary: yesterday's numbers, what needs attention
    today (low stock, money owed), and the day's best seller."""
    today = datetime.date.today()
    yesterday = today - datetime.timedelta(days=1)

    sales = db_get_sales_summary(user_id, str(yesterday), str(yesterday))
    pl = db_get_profit_loss(user_id, str(yesterday), str(yesterday))
    low_stock = db_get_low_stock_products(user_id)
    home = db_get_home_summary(user_id)
    ranking = db_get_product_sales_ranking(user_id, str(yesterday), str(yesterday))
    top_yesterday = [p for p in ranking if p["units_sold"] > 0]
    top_yesterday.sort(key=lambda p: p["units_sold"], reverse=True)
    best_seller = top_yesterday[0]["name"] if top_yesterday else "no sales recorded"
    ...
    user_prompt = (
        f"Yesterday ({yesterday}): revenue {sales['revenue']:.2f}, collected {sales['collected']:.2f}, "
...
def ai_coach_reply(user_id: int, message: str, history: list):
    ...
    pl = db_get_profit_loss(user_id, str(month_start), str(today))
    revenue, cost, profit = pl["revenue"], pl["cost"], pl["profit"]
    home = db_get_home_summary(user_id)
    ...
    api_key = get_api_key()
    if not api_key:
        return ("No Groq API key configured. Set the GROQ_API_KEY environment variable on the "
...
async def ensure_db_ready(request, call_next):
    # Serverless-safe lazy init: runs once per warm container instead of relying
    # on a startup event, which some serverless platforms don't invoke reliably.
    global _db_ready
    if not _db_ready:
        init_db()
        ensure_demo_account()
...
def register(payload: RegisterRequest):
    user_id = db_create_user(payload.business_name.strip(), payload.email, payload.password)
    if user_id is None:
...
def login(payload: LoginRequest):
    user = db_verify_login(payload.email, payload.password)
    if user is None:
...
def demo_login():
    """Logs straight into the pre-seeded demo account — wire this up to a
    'Try the demo' button on the landing page."""
    user = db_get_user_by_email(DEMO_EMAIL)
    if user is None:
        raise HTTPException(status_code=503, detail="Demo account isn't ready yet, try again in a moment.")
    token = db_create_session(user["id"])
    return {"token": token, "business_name": user["business_name"], "email": user["email"]}
...
def create_product(payload: ProductCreate, user_id: int = Depends(uid)):
    ...
    result = {"id": pid, **payload.model_dump()}
    if warning:
...
def ai_sales_insights(start_date: str, end_date: str, user_id: int = Depends(uid)):
    pl = db_get_profit_loss(user_id, start_date, end_date)
    ...
    return {"insight": analyze_sales_and_dashboard(pl["revenue"], pl["cost"], pl["profit"], top, slow)}
...
def ai_daily_brief(user_id: int = Depends(uid)):
    return {"brief": generate_daily_brief(user_id)}
